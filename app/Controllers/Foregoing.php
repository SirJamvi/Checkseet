...
class Foregoing extends BaseController
{
    ...
    public function index(): string
    {
        $data = [
            ...
            'alldata' => $this->ForegoingModel->getAllData()
        ];
 ...
 public function data()
    ...
    public function formEdit()
    {
        $data = [
            'title' => 'History | Startup Management',
            'alldata' => $this->ForegoingModel->getByNumber($_GET['number']),
            'number' => $_GET['number']
        ];
        return view("/layout/".$data['alldata'][0]['device']."/input/foregoing/".$data['alldata'][0]['process'],$data);
    }
    ...
    public function edit($number)
    {
        ...
        return view('edit-foregoing',$data);
    }

    public function formInputForegoing()
    {
        ...
        return view('/layout/'.$_GET['device']."/input/foregoing/".$_GET['process'],$data);
    }

    public function createForegoing()
    {
        if(!$this->session->has('empid')){
            return redirect()->to(base_url()."login");
        }
        $data = [
            'title' => 'Create Foregoing',
            'validation' => \Config\Services::validation()
        ];
        

        $number = $this->request->getVar('number-edit') ? $this->request->getVar('number-edit') : ($this->ForegoingModel->getLatestId()+1);
        ...
        $resulta = $this->ForegoingModel->addProses
        (
            [
                'number' => $number,
                'par000' => 1,
        ...
        $resultb = $this->ForegoingModel->addProses
        (
            [
                'number' => $number,
                'par000' => 2,
        ...
        $resultc = $this->ForegoingModel->addProses(
            [
                'number' => $number,
                'par000' => 3,
                'device' => $this->request->getVar('device-txt'),
        ...
        $resultd = $this->ForegoingModel->addProses
        (
            [
                'number' => $number,
                'par000' => 4,
        ...
        $resulte = $this->ForegoingModel->addProses
        (
            [
                'number' => $number,
                'par000' => 5,
